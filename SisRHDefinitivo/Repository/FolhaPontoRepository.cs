...
namespace SisRHDefinitivo.Repository
{
    public class FolhaPontoRepository : Conexao
    {
        ...
        public List<FolhaPontoModel> ConsultarFolhaPonto(int matricula, int mes, int dia, int ano)
        {
            try
            {
                ConectarBanco();
                ...
                using (SqlDataReader dr = cmd.ExecuteReader())
                {
                    ...
                    List<FolhaPontoModel> lista = new List<FolhaPontoModel>();

                    foreach (DataRow row in folha.Rows)
                    {
                        FolhaPontoModel model = new FolhaPontoModel();
